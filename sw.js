  ...
  event.waitUntil(
    self.clients
      ...
      .then(function (winList) {
        // Prefer focusing an already-open AnEspresso window.
        for (var i = 0; i < winList.length; i++) {
          var w = winList[i];
          if (w && w.url && w.url.indexOf('/AnEspresso-auth') !== -1) {
            if ('focus' in w) {
              return w.focus();
            }
          }
        }
        // Otherwise open the app. './' resolves against the worker's
        // scope (/AnEspresso-auth/); the app restores its Firebase
        // config from localStorage on load.
